...
export class PerformanceMeasurementModifier extends CustomChartModifier2D {
  ...
  protected createControlBarAnnotation() {
    ...
    exportFilteredButton.addEventListener(
      "click",
      exportFilteredButtonClickHandler
    );
    ...
    this.cleanupControlBar = () => {
      ...
      exportFilteredButton.removeEventListener(
        "click",
        exportFilteredButtonClickHandler
      );
  ...
  protected exportPerformanceData() {
    ...
    link.href = url;
    link.download = `profiler-data-all-${new Date()
      .toISOString()
      .replace(/[:.]/g, "-")}.json`;
  ...
  protected exportFilteredPerformanceData() {
    ...
    const filteredMarks = surfaceRelatedMarks.filter((mark) => {
      const markType = mark.name
        .split(helper.separator)[0]
        .replace(/Start|End/, "");
    ...
    const filteredData = {
      ...fullData,
      marks: filteredMarks.map(({ name, startTime, detail }) => ({
        name,
        startTime,
        detail,
      })),
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `profiler-data-filtered-${
      this.parentSurface.id
  ...
  const cleanup = () => {
    ...
    if (markTypesSelectResult.documentClickHandler) {
      document.removeEventListener(
        "click",
        markTypesSelectResult.documentClickHandler
      );
    }
  };
