...
export const createPerformanceChart = async (
  ...
  const renderTimeMountainSeries = new FastMountainRenderableSeries(
    wasmContext,
    {
  ...
  const timeSinceLastPaintMountainSeries = new FastMountainRenderableSeries(
    wasmContext,
    {
  ...
  sciChartSurface.renderableSeries.add(
    timeSinceLastPaintMountainSeries,
    renderTimeMountainSeries
  );
  ...
  const addRenderTimePoint = (time: number) => {
    ...
    renderTimeAxisLabelAnnotation.formattedValue = `Render ${time.toFixed(
      2
    )}ms`;
  };
  ...
  const addTimeSinceLastPaintPoint = (time: number) => {
    ...
    timeSinceLastPaintAxisLabelAnnotation.formattedValue = `Paint Interval ${time.toFixed(
      2
    )}ms`;
  };
