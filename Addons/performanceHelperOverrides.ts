...
export class SCPerformanceMeasure implements PerformanceMeasure {
  ...
  constructor(
    name: string,
    startMark: SCPerformanceMark,
    endMark: SCPerformanceMark
  ) {
    this.name = name;
...
export class CustomPerformanceDebugHelper extends PerformanceDebugHelper {
  ...
  public override createMark<TDetail extends TPerformanceDetail>(
    ...
    const mark = new PerformanceMark(`${type}${this.separator}${groupId}`, {
      detail,
    });
    this.marks.add(mark);
    return mark;
  ...
  protected constructMeasure<TDetail extends TPerformanceDetail>(
    ...
    const startMark = this.marks
      .asArray()
      .find((m) => m.name === startTypeName);
  ...
  protected endRecording(
    options: TRecordingOptions,
    previousTrackedMarkTypes: (EPerformanceMarkType | string)[]
  ) {
    const surface = options.sciChartSurface;
    ...
    const surfaceRelatedEndMarks = surfaceRelatedMarks.filter((mark) =>
      mark.name.includes("End")
    );

    // these are not added to any collection automatically
    const measures = surfaceRelatedEndMarks.map((endMark) =>
      this.constructMeasure(endMark)
    );
  ...
  public async recordSingleRedrawRequest(
    options: TRecordingOptions & { includeInternalRedrawRequests?: boolean }
  ) {
    const prevMarkTypes = this.startRecording(options);
    ...
    if (options.includeInternalRedrawRequests) {
      ...
      const isInvalidated = await receiveNextEvent(
        options.sciChartSurface.painted
      );
      if (isInvalidated) {
