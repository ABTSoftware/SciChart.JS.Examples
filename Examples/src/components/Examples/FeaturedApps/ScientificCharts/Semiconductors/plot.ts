...
export const drawPlot = async (
    ...
    const initSubChart = (subChartIndex: number, selectedPoint: WaferLotData) => {
        ...
        // Add click handling for wafer selection
        if (onWaferClick) {
            // Create a custom modifier to handle clicks on this subchart
            const clickModifier = new (class extends ZoomPanModifier {
                public modifierMouseDown(args: ModifierMouseArgs): boolean {
                    // Call the wafer selection callback when this subchart is clicked
                    onWaferClick(subChartIndex);
            ...
            subChartSurface.chartModifiers.add(clickModifier);
        }
