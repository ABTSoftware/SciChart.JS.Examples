...
import { GammaPoint, SmithState, SmithAction, Marker, DragMode } from "./useSmithChart";
...
// ── Constrained drag helpers ──────────────────────────────────────────────────

function projectToCircle(px: number, py: number, cx: number, cy: number, radius: number): GammaPoint {
    const dx = px - cx;
    const dy = py - cy;
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist < 1e-10) return { re: cx + radius, im: cy };
    return { re: cx + (dx / dist) * radius, im: cy + (dy / dist) * radius };
}
...
function applyConstraint(re: number, im: number, mode: DragMode, start: DragStart): GammaPoint {
    switch (mode) {
        case "gamma": {
            const mag = Math.sqrt(re * re + im * im);
            if (mag < 1e-10) return { re: start.gammaMag, im: 0 };
            return { re: (re / mag) * start.gammaMag, im: (im / mag) * start.gammaMag };
        }
        case "R": {
            const cx = start.r / (1 + start.r);
            return projectToCircle(re, im, cx, 0, 1 / (1 + start.r));
        }
        case "X": {
            const absX = Math.abs(start.x);
            if (absX < 0.001) return { re, im: 0 };
            const cy = start.x > 0 ? 1 / absX : -1 / absX;
            return projectToCircle(re, im, 1, cy, 1 / absX);
        }
        case "G": {
            const cx = -(start.g / (1 + start.g));
            return projectToCircle(re, im, cx, 0, 1 / (1 + start.g));
        }
        case "B": {
            const absB = Math.abs(start.b);
            if (absB < 0.001) return { re, im: 0 };
            const cy = start.b > 0 ? 1 / absB : -1 / absB;
            return projectToCircle(re, im, -1, cy, 1 / absB);
        }
        default:
            return { re, im };
    }
...
class SmithClickModifier extends ChartModifierBase2D {
    ...
    private markerPositions: Map<string, { re: number; im: number; dragMode: DragMode }> = new Map();
    private dragTargetId: string | null = null;

    // Captured at drag start for constraint projection
    private dragStart: DragStart = { gammaMag: 0, r: 0, x: 0, g: 0, b: 0 };
    private dragStartMode: DragMode = "free";

    setDispatch(d: (a: SmithAction) => void) {
        this.dispatch = d;
    }

    setMarkerPositions(positions: Map<string, { re: number; im: number; dragMode: DragMode }>) {
    ...
    modifierMouseDown(args: ModifierMouseArgs) {
        ...
        if (hitId !== null) {
            ...
            const md = this.markerPositions.get(hitId);
            if (md) {
                this.dragStartMode = md.dragMode;
                const { re: mr, im: mi } = md;
                const dz = (1 - mr) ** 2 + mi ** 2;
                const zr = dz > 1e-10 ? (1 - mr * mr - mi * mi) / dz : 1e6;
                const zx = dz > 1e-10 ? (2 * mi) / dz : 0;
                const zm2 = zr ** 2 + zx ** 2;
                this.dragStart = {
                    gammaMag: Math.sqrt(mr * mr + mi * mi),
                    r: zr,
                    x: zx,
                    g: zm2 > 1e-10 ? zr / zm2 : 1e6,
                    b: zm2 > 1e-10 ? -zx / zm2 : 0,
                };
            } else {
                this.dragStartMode = "free";
            }
    ...
    private hitTestMarkers(px: number, py: number): string | null {
        ...
        for (const [id, pos] of this.markerPositions) {
            ...
            const dx = localX - sx;
            const dy = localY - sy;
            if (dx * dx + dy * dy <= RADIUS_PX * RADIUS_PX) return id;
        }
        return null;
    }
}
...
export class SmithMarkersAdapter {
    ...
    private syncMarkerAnnotations(markers: Marker[]) {
        ...
        const markerPositions = new Map<string, { re: number; im: number; dragMode: DragMode }>();

        for (let i = 0; i < markers.length; i++) {
            const marker = markers[i];
            const color = MARKER_COLOURS[i % MARKER_COLOURS.length];
            markerPositions.set(marker.id, { re: marker.gamma.re, im: marker.gamma.im, dragMode: marker.dragMode });
    ...
    private updateHighlights(state: SmithState) {
        const rAxis = this.surface.xAxes.get(0) as SmithChartResistanceAxis;
        if (!state.activeMarkerId) {
            this.rCircleDS.clear();
            this.xArcDS.clear();
            this.gammaCircleDS.clear();
            rAxis.selectedIntersection = null;
            return;
        }
        ...
        // Drive intersection labels on the axis renderers — use exact marker R/X
        // so labels align with the visual highlighted arcs (not snapped major values).
        rAxis.selectedIntersection = isFinite(r) && r >= 0 && isFinite(x) ? { r, x } : null;
    }
}
