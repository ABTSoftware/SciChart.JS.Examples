...
export type DragMode = "free" | "gamma" | "R" | "X" | "G" | "B";
...
export type SmithAction =
    ...
    | { type: "SET_VSWR_OUTLINE"; outline: boolean }
    ...
    | { type: "CLEAR" }
    | { type: "LOAD_SCENARIO"; id: string; steps: string[] }
    | { type: "SET_DRAG_MODE"; id: string; mode: DragMode };
...
export function smithReducer(state: SmithState, action: SmithAction): SmithState {
    switch (action.type) {
        ...
        case "SET_VSWR_OUTLINE":
            return { ...state, vswrOutline: action.outline };
        case "SET_GRID_MODE":
        ...
        case "CLEAR":
            return initialSmithState();
        case "LOAD_SCENARIO":
            return { ...state, activeScenarioId: action.id, scenarioSteps: action.steps };
        case "SET_DRAG_MODE":
            return {
                ...state,
                markers: state.markers.map((m) => (m.id === action.id ? { ...m, dragMode: action.mode } : m)),
            };
        default:
            return state;
    }
