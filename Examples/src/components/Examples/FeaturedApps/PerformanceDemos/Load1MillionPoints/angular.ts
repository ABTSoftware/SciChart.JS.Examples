      ...
      <ng-container *ngIf="timeSpans.length > 0">
        <mat-card class="notification-card">
          ...
          <mat-card-content class="notification-card-content">
            <div *ngFor="let ts of timeSpans; let i = index" [attr.key]="i" class="performance-result">
...
export class Load1MillionPointsChartComponent implements OnInit, OnDestroy {
  timeSpans: TTimeSpan[] = [];
  ...
  async onInit(initResult: any) {
    ...
    this.initResult.controls.loadPoints(this.updateTimeSpans.bind(this));
  }
