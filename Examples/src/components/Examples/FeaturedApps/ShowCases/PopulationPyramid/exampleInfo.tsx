    ...
    //// This metadata is computer generated - do not edit!
    {
        ...
        frameworks: {
            javascript: {
                ...
                markdownContent:
                    "## Population Pyramid Example in JavaScript\n\n### Overview\nThis example demonstrates how to create a high-performance population pyramid chart using SciChart.js in JavaScript. It visualizes demographic data for regions such as Africa and Europe using stacked column charts with synchronized axes, all implemented without any additional frameworks.\n\n### Technical Implementation\nThe chart is initialized by creating a `SciChartSurface` along with a WebAssembly context (`wasmContext`), leveraging techniques described in the [Creating a new SciChartSurface and loading Wasm](https://www.scichart.com/documentation/js/v5/2d-charts/surface/new-scichart-surface/) documentation. NumericAxis instances are configured with custom properties and their visible ranges are synchronized as detailed in the [Axis Ranging - Setting and Getting VisibleRange](https://www.scichart.com/documentation/js/v5/2d-charts/axis-api/ranging-scaling/set-range-zoom-to-fit/) guide. A `CustomDataLabelManager` implementing `IDataLabelLayoutManager` is implemented to avoid overlapping labels, following strategies explained in [Data Label Positioning](https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/data-point-labels/data-label-positioning/). The population pyramid is constructed by combining `StackedColumnRenderableSeries` into a `StackedColumnCollection`, as described in [The Stacked Column Series Type](https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/stacked-column-renderable-series/). Additionally, smooth series transitions are achieved using `WaveAnimation`, as covered by the [Animations API](https://www.scichart.com/documentation/js/v5/2d-charts/animations-api/animations-api-overview/).\n\n### Features and Capabilities\nThe example supports interactive zooming and panning through modifiers such as `ZoomExtentsModifier` and `MouseWheelZoomModifier`, providing an engaging user experience noted in [Tutorial 03 - Adding Zooming, Panning Behavior](https://www.scichart.com/documentation/js/v5/get-started/tutorials-js-npm-webpack/tutorial-03-adding-zooming-panning-behavior/). It also includes advanced capabilities like selective legend configuration and the synchronization of dual axes for male and female data series within the stacked column chart.\n\n### Integration and Best Practices\nIn this JavaScript implementation, resource cleanup is handled by calling `sciChartSurface.delete()`, following best practices outlined in [Memory Best Practices](https://www.scichart.com/documentation/js/v5/2d-charts/performance-tips/memory-best-practices/). Although SciChart.js can be integrated with frameworks like Angular and React, this example focuses solely on JavaScript, offering a lightweight yet powerful solution for building real-time, high-performance charts with extensive customization options.",
            },
            react: {
                subtitle:
                    "Population Pyramid of Europe and Africa using SciChart.js High Performance JavaScript Charts. This also demonstrates the use of DataLabelLayoutManager to Modify the positions of data labels from different series to prevent overlap",
            ...
            angular: {
                ...
                metaDescription: "Population Pyramid of Europe and Africa",
                markdownContent:
                    "## Angular Population Pyramid Example\n\n### Overview\nThis Angular example demonstrates a highly interactive Population Pyramid chart using SciChart.js. The chart visualizes demographic data for Europe and Africa by displaying separate stacked column series for male and female populations. A custom data label layout manager is implemented to prevent overlapping labels, ensuring that the demographic data remains clear and easily interpretable.\n\n### Technical Implementation\nThe chart is initialized within an Angular standalone component by leveraging the `ScichartAngularComponent`. The example creates an X axis for age groups and two Y axes that represent male and female populations respectively. These Y axes are synchronized using Angular event subscriptions, a process detailed in the [Synchronizing Multiple Charts](https://www.scichart.com/documentation/js/v5/2d-charts/chart-synchronization-api/synchronizing-multiple-charts/) documentation. A custom data label layout manager is used to dynamically adjust label positions to avoid overlap; this approach aligns with the best practices outlined in the [Data Label Positioning](https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/data-point-labels/data-label-positioning/) guide. Additionally, the integration of wave animations for the stacked column series is optimized for performance, as described in the [Animations API](https://www.scichart.com/documentation/js/v5/2d-charts/animations-api/animations-api-overview/).\n\n### Features and Capabilities\nThe example offers robust interactive capabilities including real-time zooming via both the `MouseWheelZoomModifier` and `ZoomExtentsModifier`, as outlined in the [MouseWheelZoomModifier](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/zooming-and-panning/mouse-wheel-zoom-modifier/) documentation. The dual Y-axis design ensures that male and female data are presented in a comparative manner while keeping both axes in sync. The implementation also supports dynamic legends with selective series inclusion, reinforcing clarity and user control over the displayed data.\n\n### Integration and Best Practices\nThis implementation follows Angular best practices by using property binding to initialize the chart in a standalone component, as referenced in the [Getting started with standalone components](https://angular.io/guide/standalone-components) documentation and the [scichart-angular](https://www.npmjs.com/package/scichart-angular) package. By combining efficient WebGL rendering with custom layout mechanisms, developers can achieve high-performance charts even with complex datasets. The approach not only adheres to modern Angular integration patterns but also demonstrates efficient techniques for managing interactive chart features and ensuring consistent, synchronized axes. Detailed examples of dual-axis configuration and event-driven synchronization can be found in the [Synchronizing Multiple Charts](https://www.scichart.com/documentation/js/v5/2d-charts/chart-synchronization-api/synchronizing-multiple-charts/) documentation.",
            },
        },
        documentationLinks: [
            {
