...
class CustomDataLabelManager implements IDataLabelLayoutManager {
    performTextLayout(sciChartSurface: SciChartSurface, renderPassInfo: RenderPassInfo): void {
        const renderableSeries = sciChartSurface.renderableSeries.asArray() as IRenderableSeries[];

        for (let i = 0; i < renderableSeries.length; i++) { // loop through all series (i.e. 2 stacked series - Male and Female)

            const currentSeries = renderableSeries[i] as StackedColumnRenderableSeries;
            if (currentSeries instanceof StackedColumnCollection) {
                // @ts-ignore
                const stackedSeries:StackedColumnRenderableSeries[] = currentSeries.asArray();
                    ...
                    for (let k = 0; k < innerLabels.length; k++) {
                        ...
                        if (outerLabel && innerLabel) {
                            ...
                            if (Math.abs(outerLabelPosition.x - innerLabelPosition.x) < limitWidth) {
                                let newX;
                                if(i == 0){ // if we are in Male (left) chart, draw left
                                    newX = innerLabel.position.x - outerLabel.rect.width - marginBetweenLabels;
                                }
                                else { // if we are in Female (right) chart, draw right
...
export const drawExample = async (rootElement: string | HTMLDivElement) => {
    ...
    sciChartSurface.renderableSeries.add(
        stackedColumnCollectionMale,
        stackedColumnCollectionFemale
    );
