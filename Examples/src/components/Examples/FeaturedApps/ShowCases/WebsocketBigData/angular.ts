    ...
    <div style="display: flex; height: 100vh;">
      ...
      <div style="width: 300px; padding: 10px;">
        <form #form="ngForm">
          ...
          <mat-slider name="seriesCount" min="1" [max]="maxSettings.seriesCount" [(ngModel)]="settings.seriesCount" (change)="handleFormChange(form)">
            <input matSliderThumb>
          </mat-slider>
          <div>Number of Series: {{ settings.seriesCount }}</div>
          <mat-slider name="initialPoints" min="0.1" [max]="maxSettings.initialPoints" [(ngModel)]="settings.initialPoints" (change)="handleFormChange(form)" [marks]="getLogMarks(maxSettings.initialPoints)">
            <input matSliderThumb>
          </mat-slider>
          <div>Initial Points: {{ settings.initialPoints }}</div>
          <mat-slider name="pointsOnChart" min="0.1" [max]="maxSettings.pointsOnChart" [(ngModel)]="settings.pointsOnChart" (change)="handleFormChange(form)" [marks]="getLogMarks(maxSettings.pointsOnChart)">
            <input matSliderThumb>
          </mat-slider>
          <div>Max Points On Chart: {{ settings.pointsOnChart }}</div>
          <mat-slider name="pointsPerUpdate" min="0.1" [max]="maxSettings.pointsPerUpdate" [(ngModel)]="settings.pointsPerUpdate" (change)="handleFormChange(form)" [marks]="getLogMarks(maxSettings.pointsPerUpdate)">
            <input matSliderThumb>
          </mat-slider>
          <div>Points Per Update: {{ settings.pointsPerUpdate }}</div>
          <mat-slider name="sendEvery" min="{{maxSettings.sendEvery}}" max="500" [(ngModel)]="settings.sendEvery" (change)="handleFormChange(form)">
            <input matSliderThumb>
          </mat-slider>
