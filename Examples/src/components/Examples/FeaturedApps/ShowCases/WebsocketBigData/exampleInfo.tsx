        ...
        frameworks: {
            ...
            react: {
                ...
                metaDescription:
                    "This demo showcases the incredible realtime performance of our JavaScript charts by updating the series with millions of data-points!",
                markdownContent:
                    "## Websocket Big Data Demo in React\n\n### Overview\nThis example demonstrates a high-performance real-time charting application using SciChart.js in a React environment. The demo handles massive streaming datasets via a WebSocket connection and dynamically updates charts with millions of data points. It is designed to showcase the capabilities of SciChart.js when integrated as a React component, providing a robust solution for real-time data visualization.\n\n### Technical Implementation\nThe implementation leverages the `<SciChartReact/>` component from the [scichart-react](https://www.npmjs.com/package/scichart-react) package to create and render the chart. Key parts of the implementation include initializing the chart with custom axes, dynamically updating settings via React state, and handling data streaming from a WebSocket. Chart updates are efficiently managed by invoking methods to `appendRange()` and `removeRange()` to manage chart data on the fly. This technique could be further improved by using `fifoCapacity` - where `XyDataSeries` are implemented with a First-In-First-Out (FIFO) circular buffer, for automatic discarding of old data. For a deeper understanding of how to integrate `SciChartSurface` with React, developers can refer to the [React Charts with SciChart.js](https://www.scichart.com/blog/react-charts-with-scichart-js/) blog post.\n\n### Features and Capabilities\nThe demo implements several advanced features including real-time updates, dynamic series type selection of **Line**, **Column**, **Band**, **Candlestick**, **Stacked Mountain** or **Scatter** Chart via dropdown controls, and performance monitoring (load times, render times, and maximum FPS calculations). Real-time data streaming is managed through `socket.io`, where incoming data is batched and processed to update the chart continuously. This efficient handling of large data streams helps illustrate best practices for real-time charting with WebGL rendering.\n\n### Integration and Best Practices\nThe example makes extensive use of React hooks such as `useState` and `useRef` to manage state and access imperative chart control methods. Dynamic configuration of the chart is achieved with Material UI sliders that incorporate logarithmic scaling for precise control over parameters like series count and data point limits. This approach follows [best practices for managing real-time chart updates](https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/data-series-api/append-insert-update-remove/) in React and illustrates how to integrate complex chart control logic seamlessly into a React component. Performance optimization is also a key focus, ensuring that heavy data loads do not hamper the user experience. Developers looking to deepen their understanding of these techniques can consult resources on [performance optimization for React Chart applications](https://www.scichart.com/documentation/js/v5/2d-charts/performance-tips/performance-tips-and-tricks/).",
            },
            angular: {
                subtitle:
                    "Demonstrates handling realtime big data with different chart types using SciChart.js, High Performance JavaScript Charts",
...
export default websocketBigDataDemoExampleInfo;
