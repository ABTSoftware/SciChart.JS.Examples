...
export const getChartsInitializationApi = () => {
    ...
    let crossSectionSurface: SciChartSurface;
    let inputSurface: SciChartSurface;
    let outputSurface: SciChartSurface;
    let addInputSeries: (color: string, freq?: number) => void;
    let addOutputSeries: (color: string) => void;
    ...
    const initCrossSectionChart = async (rootElement: string | HTMLDivElement) => {
        const { sciChartSurface, wasmContext } = await SciChartSurface.create(rootElement, {
            theme: appTheme.SciChartJsTheme,
        });
        ...
        crossSectionSurface = sciChartSurface;

        return { sciChartSurface };
    };

    const inputChart = async (rootElement: string | HTMLDivElement) => {
        const { sciChartSurface, wasmContext } = await SciChartSurface.create(rootElement, {
            theme: appTheme.SciChartJsTheme,
        });
        ...
        const makeYValues = (frequency: number) => {
            return Array.from(Array(60)).map(
                (_, i) => Math.sin(((frequency * 2 - 30000) * i * 2 * Math.PI) / 60000) * 80
            );
        };

        const addInputSeriesCallback = (color: string, freq?: number) => {
        ...
        inputSurface = sciChartSurface;
        addInputSeries = addInputSeriesCallback;

        return { sciChartSurface, addInputSeries: addInputSeriesCallback };
    };

    const initHistoryChart = async (rootElement: string | HTMLDivElement) => {
        const { sciChartSurface, wasmContext } = await SciChartSurface.create(rootElement, {
            theme: appTheme.SciChartJsTheme,
        });
        const xAxis = new NumericAxis(wasmContext, {
        ...
        const addOutputSeriesCallback = (color: string) => {
        ...
        addOutputSeries = addOutputSeriesCallback;

        return { sciChartSurface, addOutputSeries: addOutputSeriesCallback };
    };

    const onAllChartsInit = () => {
        const heatmapDataSeries = new UniformHeatmapDataSeries(mainSurface.webAssemblyContext2D, {
            xStart: 1,
            xStep: 1,
            yStart: 1,
            yStep: 1,
        ...
        const heatmapSeries = new UniformHeatmapRenderableSeries(mainSurface.webAssemblyContext2D, {
            opacity: 0.8,
        ...
        mainSurface.renderableSeries.add(heatmapSeries);
        ...
        const removeInput = (input: BoxAnnotation) => {
            mainSurface.annotations.remove(input);
        ...
        const addInput = (x: number, y: number, w?: number, h?: number, freq?: number) => {
            ...
            mainSurface.annotations.add(boxInput);
            addInputSeries(color, freq);
        };

        const removeOutput = (output: CustomAnnotation) => {
            mainSurface.annotations.remove(output);
        ...
        const addOutput = (x: number, y: number) => {
            ...
            outputs.push(output);
            mainSurface.annotations.add(output);
            addOutputSeries(color);
        };

        mainSurface.annotations.add(hline, vline);

        addIO.onAddInput = addInput;
        addIO.onAddOutput = addOutput;
        mainSurface.chartModifiers.add(addIO);
        ...
        mainSurface.annotations.add(dampingMarker);

        mainSurface.annotations.add(
            new NativeTextAnnotation({
                x1: 0.5,
                y1: 0.02,
                xCoordinateMode: ECoordinateMode.Relative,
        ...
        const showHelp = () => {
            ...
            mainSurface.addAnimation(anim.startAnim);
            ...
            anim.next.onNext = () => mainSurface.addAnimation(anim2.startAnim);
            ...
            anim2.next.onNext = () => mainSurface.addAnimation(anim3.startAnim);
            ...
            anim3.next.onNext = () => mainSurface.addAnimation(anim4.startAnim);
            ...
            anim4.next.onNext = () => mainSurface.addAnimation(anim5.startAnim);
            ...
            anim5.next.onNext = () => mainSurface.addAnimation(anim6.startAnim);
        };
        showHelp();
