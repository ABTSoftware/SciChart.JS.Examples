...
export const drawExample = async () => {
    ...
    const lineDataSeries = crossSectionSurface.renderableSeries.getById("h").dataSeries;
    const vlineDataSeries = crossSectionSurface.renderableSeries.getById("v").dataSeries;
    ...
    const addInput = (x, y, w, h, freq) => {
        ...
        boxInput.selectedChanged.subscribe((isSelected) => {
    ...
    const addOutput = (x, y) => {
        ...
        output.selectedChanged.subscribe((isSelected) => {
            if (isSelected) {
    ...
    const updateChart = () => {
        ...
        const newZValues = Array.from(Array(height), (_) => Array(width));
        ...
        for (const boxInput of inputs) {
            const inputSeries = inputSurface.renderableSeries.getById(boxInput.id).dataSeries;
        ...
        for (const output of outputs) {
            const outputDS = outputSurface.renderableSeries.getById(output.id).dataSeries;
...
const getHelpAnnotation = (text, surface) => {
    ...
    const startAnim = new GenericAnimation({
        from: 0,
        to: 1,
        duration: 500,
...
class YPalette extends BasePaletteProvider {
    strokePaletteMode;
    dataSeries;
    wasmContext;
    colorData;
    constructor(wasmContext, stops) {
        super();
        this.wasmContext = wasmContext;
        this.colorData = PaletteFactory.createColorMap(wasmContext, stops);
    }
    onAttached(parentSeries) {
        this.dataSeries = parentSeries.dataSeries;
    }
    onDetached() {}
