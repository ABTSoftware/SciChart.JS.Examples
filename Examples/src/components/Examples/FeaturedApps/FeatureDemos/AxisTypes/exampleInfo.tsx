    ...
    //// This metadata is computer generated - do not edit!
    {
        ...
        frameworks: {
            javascript: {
                ...
                markdownContent:
                    "## Axis Types Example in JavaScript\n\n### Overview\nThis example demonstrates how to configure and integrate different axis types in SciChart.js using JavaScript. It combines a variety of axes including a `CategoryAxis` with string text labels provided by a `TextLabelProvider`, a `NumericAxis` for numerical data with prefix and postfix formatting, a `DateTimeNumericAxis` that auto-formats dates based on the date range, and a `LogarithmicAxis` for log-scaled data.\n\n### Technical Implementation\nThe chart is constructed by creating a new `SciChartSurface` along with a WebAssembly context using the function `SciChartSurface.create()`. Multiple axes are programmatically added by instantiating their respective classes, for example, the `CategoryAxis` and `DateTimeNumericAxis` (see [DateTimeNumericAxis](https://www.scichart.com/documentation/js/v4/2d-charts/axis-api/axis-types/date-time-numeric-axis/) for details). A custom palette provider (`AxisTypesPaletteProvider`) is implemented by overriding methods such as `overrideFillArgb`, in line with the guidelines provided in the [PaletteProvider documentation](https://www.scichart.com/documentation/js/v4/2d-charts/chart-types/palette-provider-api/palette-provider-api-overview/).\n\n### Features and Capabilities\nThe example emphasizes high-performance rendering using fast renderable series like `FastColumnRenderableSeries` and `FastLineRenderableSeries`; these components are optimized for large datasets as described in the [FastColumnRenderableSeries documentation](https://www.scichart.com/documentation/js/v4/typedoc/classes/fastcolumnrenderableseries.html). It also demonstrates asynchronous data integration, where real-time data is fetched and appended to `XyDataSeries` ([DataSeries Append/Update](https://www.scichart.com/documentation/js/v4/2d-charts/chart-types/data-series-api/append-insert-update-remove/)), ensuring the chart remains responsive. Additionally, interactive chart modifiers such as `ZoomPanModifier`, `MouseWheelZoomModifier`, and `ZoomExtentsModifier` are incorporated to support dynamic zooming and panning functions (refer to [Adding Zooming, Panning Behavior](https://www.scichart.com/documentation/js/v4/get-started/tutorials-js-npm-webpack/tutorial-03-adding-zooming-panning-behavior/) for more information).\n\n### Integration and Best Practices\nAlthough this example is implemented in JavaScript, it is designed to be easily integrated into other frameworks, as seen in the accompanying React integration via the `<SciChartReact/>` component. Developers are encouraged to follow performance optimization techniques such as leveraging WebGL and WebAssembly for rapid rendering—topics covered in the [Performance Tips & Tricks](https://www.scichart.com/documentation/js/v4/2d-charts/performance-tips/performance-tips-and-tricks/) documentation. Furthermore, custom axis labeling is achieved using the `TextLabelProvider`, enabling developers to fully customize how string labels are displayed on a `CategoryAxis` (see [TextLabelProvider](https://www.scichart.com/documentation/js/v4/typedoc/classes/textlabelprovider.html) for additional context). Overall, the example illustrates advanced features like real-time updates and custom styling, providing a comprehensive foundation for building high-performance financial charting applications with SciChart.js.",
            },
            react: {
                subtitle:
                    "Demonstrates the Numeric, Category, Date and Logarithmic axis types available SciChart.js, High Performance JavaScript Charts",
                ...
                metaDescription:
                    "Demonstrates how to use arbitrary text for axis labels, rather than formatted data values, using the new TextLabelProvider",
                markdownContent:
                    "## Axis Types Example - React\n\n### Overview\nThis example demonstrates the integration of SciChart.js within a React application using the `<SciChartReact/>` component. It showcases multiple axis types including `CategoryAxis`, `NumericAxis`, `DateTimeNumericAxis` and `LogarithmicAxis`. Custom axis labeling is implemented using the `TextLabelProvider` to display arbitrary string text labels, and a custom palette provider (`AxisTypesPaletteProvider`) is used for dynamic coloring of columns.\n\n### Technical Implementation\nThe chart is initialized in a React component via the `<SciChartReact/>` element, which encapsulates the setup within React lifecycle methods. All axis types are instantiated directly and configured with specific properties such as custom font sizes, alignments, and prefixes/postfixes for labels. Developers can refer to the [SciChart.js Axis Documentation](https://www.scichart.com/documentation/js/v4/2d-charts/axis-api/axis-api-overview/) for a detailed explanation on the various axis types. Custom axis labeling is achieved through the TextLabelProvider as explained in the [Axis LabelProviders Documentation](https://www.scichart.com/documentation/js/v4/2d-charts/axis-api/axis-labels/label-provider-api-overview/). Additionally, the example utilizes a custom palette provider by extending `IStrokePaletteProvider` and `IFillPaletteProvider` for dynamic series styling.\n\n### Features and Capabilities\nThe example includes several advanced features and capabilities such as real-time data updates that fetch and render data asynchronously, interactive chart behaviors with zoom, pan and mouse wheel actions, and multi-axis configurations that allow parallel display of datasets with different scales. Real-time updates are managed in a React-friendly way, consolidating state management with SciChart.js’s high-performance WebGL rendering, ensuring smooth updates even with large datasets, as detailed in the [Adding Realtime Updates](https://www.scichart.com/documentation/js/v4/get-started/tutorials-js-npm-webpack/tutorial-04-adding-realtime-updates/) guide.\n\n### Integration and Best Practices\nThe integration with React follows best practices by utilizing the `<SciChartReact/>` component which ensures proper initialization and cleanup through React’s lifecycle, as discussed in [Creating a SciChart React Component from the Ground Up](https://www.scichart.com/documentation/js/v4/get-started/tutorials-react/tutorial-01-setting-up-project-with-scichart-react/). Furthermore, the example employs performance optimization techniques by leveraging WebGL rendering capabilities, as outlined in the [Performance Tips & Tricks](https://www.scichart.com/documentation/js/v4/2d-charts/performance-tips/performance-tips-and-tricks/) documentation. Developers building production-grade charting solutions in React can benefit from the modular design of this example which promotes reusability and efficient resource management, making it easier to integrate complex chart configurations within larger applications. For further insights into the integration of SciChart.js with React, check out [React Charts with SciChart.js: Introducing “SciChart React”](https://www.scichart.com/blog/react-charts-with-scichart-js/).",
            },
            angular: {
                subtitle:
                    "Demonstrates the Numeric, Category, Date and Logarithmic axis types available SciChart.js, High Performance JavaScript Charts",
