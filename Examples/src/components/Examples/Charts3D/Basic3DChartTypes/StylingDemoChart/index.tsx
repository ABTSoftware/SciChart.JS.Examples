...
export default function StylingDemoChart() {
    ...
    const [planeDrawTitlesMode, setPlaneDrawTitlesMode] = useState<EAxisPlaneDrawLabelsMode>(EAxisPlaneDrawLabelsMode.Both);
    const [planeDrawLabelsMode, setPlaneDrawLabelsMode] = useState<EAxisPlaneDrawLabelsMode>(EAxisPlaneDrawLabelsMode.Both);
    ...
    const handlePlaneIsVisible = (e: SelectChangeEvent<string>) => {
        ...
        controlsRef.current?.setIsPlaneVisible(selectedPlane, newValue === "true");
    };
    ...
    const controlPanel = (
        <>
            ...
            <Accordion expanded={expanded === "panel1"} onChange={handleChange("panel1")} sx={accordionStyle}>
                ...
                <AccordionDetails>
                    <FormControl fullWidth className={commonClasses.formControl}>
                        <Typography variant="inherit" className={commonClasses.FormControlLabel}>Select Axis</Typography>
                        ...
                        <Typography variant="inherit" sx={{ mt: 2 }}>Axis Font Size: {currentSettings.fontSize}</Typography>
                        <Slider step={1} min={10} max={30} value={currentSettings.fontSize} onChange={handleLabelFontSize} />

                        <Typography variant="inherit">Axis Title Offset: {currentSettings.titleOffset}</Typography>
                        <Slider step={1} min={0} max={100} value={currentSettings.titleOffset} onChange={handleTitleOffset} />

                        <Typography variant="inherit">Tick Labels Offset: {currentSettings.tickLabelsOffset}</Typography>
                        <Slider step={1} min={0} max={100} value={currentSettings.tickLabelsOffset} onChange={handleTickLabelsOffset} />

                        <Typography variant="inherit">Label Orientation Mode</Typography>
                        <Select variant="standard" sx={selectStyle} value={currentSettings.labelOrientation} onChange={handleLabelOrientationModeChange}>
                        ...
                        <Stack direction="row" justifyContent="space-between" sx={{ mt: 1 }}>
                            <FormControlLabel control={<Switch checked={currentSettings.majorGridLines} onChange={handleEnableMajorGridLines} />} label="Major Grid" />
                            <FormControlLabel control={<Switch checked={currentSettings.minorGridLines} onChange={handleEnableMinorGridLines} />} label="Minor Grid" />
                        </Stack>

                        <Typography variant="inherit" sx={{ mt: 2 }}>Colors (Bands, Major, Minor)</Typography>
                        <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
                            <input type="color" value={formatHexForInput(currentSettings.bandsFill)} onChange={handleBandsFillChange} style={colorInputStyle} />
                            <input type="color" value={formatHexForInput(currentSettings.majorGridColor)} onChange={handleMajorGridLineColorChange} style={colorInputStyle} />
                            <input type="color" value={formatHexForInput(currentSettings.minorGridColor)} onChange={handleMinorGridLineColorChange} style={colorInputStyle} />
                        </Stack>
                    </FormControl>
                </AccordionDetails>
            </Accordion>
            ...
            <Accordion expanded={expanded === "panel2"} onChange={handleChange("panel2")} sx={accordionStyle}>
                ...
                <AccordionDetails>
                    ...
                    <Select fullWidth variant="standard" sx={selectStyle} value={selectedPlane} onChange={handlePlaneChange}>
    ...
    return (
        <Stack ref={sizeRef} sx={{ width: "100%", height: "100%", background: appTheme.DarkIndigo }} direction={isMobileView ? "column" : "row"}>
            <SciChartReact
            ...
            <div style={isMobileView ? mobileContainerStyle : desktopContainerStyle}>
                {isMobileView && (
                    <IconButton onClick={handleClickOpen} sx={{ color: appTheme.ForegroundColor }}><SettingsIcon fontSize="large" /></IconButton>
                )}
                {!isMobileView && controlPanel}
                {isMobileView && (
                    <Dialog open={isDialogOpen} onClose={handleClose} sx={{ "& .MuiDialog-paper": { background: appTheme.DarkIndigo } }}>
                        <DialogTitle sx={{ color: appTheme.ForegroundColor, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            Configuration
                            <IconButton onClick={handleClose} sx={{ color: 'grey.500' }}><CloseIcon /></IconButton>
                        </DialogTitle>
...
const accordionStyle = {
    ...
    "&:before": { display: "none" },
    "& .MuiAccordionSummary-root": { backgroundColor: appTheme.Indigo, borderBottom: `1px solid ${appTheme.VividSkyBlue}22` },
...
const desktopContainerStyle: React.CSSProperties = { flex: "none", width: "300px", padding: "0 10px", overflowY: "auto", color: appTheme.ForegroundColor, fontSize: "0.8em" };
