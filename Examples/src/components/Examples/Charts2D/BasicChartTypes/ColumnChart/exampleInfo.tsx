    ...
    //// This metadata is computer generated - do not edit!
    {
        ...
        frameworks: {
            ...
            react: {
                ...
                markdownContent:
                    "## React Column Chart Example\n\n### Overview\nThis example demonstrates how to integrate SciChart.js with React to create a **Column Chart** featuring data labels, rounded corners, gradient fill, and startup animations. The chart is rendered using the `<SciChartReact/>` component, ensuring a seamless integration within a React application.\n\n### Technical Implementation\nThe chart is initialized through an asynchronous `drawExample` function, which creates a `SciChartSurface` and sets up numeric axes. A `FastColumnRenderableSeries` is added with an `XyDataSeries` for x and y values. Key features such as the gradient palette provider for advanced fill styling and a `WaveAnimation` for smooth startup animations are integrated directly in the series configuration. This approach is detailed further in the [Column Series Type Documentation](https://www.scichart.com/documentation/js/v4/2d-charts/chart-types/fast-column-renderable-series/column-series-type/).\n\n### Features and Capabilities\nAdvanced styling is achieved with options like rounded corners, a gradient fill created via the palette provider, and configurable data labels that position text above the columns as described in the [Adding DataLabels to a Chart Series](https://www.scichart.com/documentation/js/v4/2d-charts/chart-types/data-point-labels/data-labels-api-overview/) guide. Interactive modifiers such as `ZoomPanModifier`, `ZoomExtentsModifier`, and `MouseWheelZoomModifier` are added to enhance user experience and enable real-time chart interactions.\n\n### Integration and Best Practices\nThe integration leverages the `<SciChartReact/>` component, ensuring best practices for embedding high-performance WebGL charts in React. The implementation follows the principles highlighted in [React Charts with SciChart.js: Introducing “SciChart React”](https://www.scichart.com/blog/react-charts-with-scichart-js/) and emphasizes proper lifecycle management, initializing and cleaning up the chart efficiently. For further guidance on setting up such projects, refer to the [Tutorial 01 - Setting up a project with scichart-react and config object](https://www.scichart.com/documentation/js/v4/get-started/tutorials-react/tutorial-01-setting-up-project-with-scichart-react/).",
            },
            angular: {
                subtitle:
                ...
                pageTitle: "Angular Column Chart",
                metaDescription:
