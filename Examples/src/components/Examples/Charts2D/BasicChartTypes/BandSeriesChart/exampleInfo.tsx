    ...
    //// This metadata is computer generated - do not edit!
    {
        ...
        frameworks: {
            javascript: {
                ...
                markdownContent:
                    "## Band Series Chart in JavaScript\n\n### Overview\nThis example demonstrates how to create a high-performance **band series chart** using SciChart.js in a JavaScript environment. It visualizes high and low values using two band series, each with distinct visual styles and smooth animations.\n\n### Technical Implementation\nThe chart is initialized asynchronously with [SciChartSurface.create](https://www.scichart.com/documentation/js/v5/2d-charts/surface/new-scichart-surface/#scichartsurfacecreate), which efficiently sets up the WebGL context. The implementation adds numeric X and Y axes and prepares three arrays for X, Y, and Y1 values. These values are fed into the `XyyDataSeries` (see [XyyDataSeries documentation](https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/data-series-api/data-series-api-overview/)) to supply the data for each band. Two `FastBandRenderableSeries` are then created. The first series applies solid color fills with transparency, while the second series demonstrates the use of linear gradient fills defined via [GradientParams](https://www.scichart.com/documentation/js/v5/typedoc/classes/gradientparams.html). Both series incorporate smooth transitioning effects using `SweepAnimation` ([SweepAnimation documentation](https://www.scichart.com/documentation/js/v5/typedoc/classes/sweepanimation.html)).\n\n### Features and Capabilities\nThis example highlights several advanced features: interactive modifiers such as `ZoomExtentsModifier`, `ZoomPanModifier`, and `MouseWheelZoomModifier` enhance user experience by providing intuitive zooming and panning (refer to the [interactive modifier guide](https://www.scichart.com/documentation/js/v5/get-started/tutorials-js-npm-webpack/tutorial-03-adding-zooming-panning-behavior/)). Additionally, the implementation demonstrates robust handling of real-time data updates and performance optimizations through efficient WebGL rendering.\n\n### Integration and Best Practices\nUsing a pure JavaScript approach, the example directly calls an asynchronous function that both creates the chart and returns a destructor for proper resource management. Developers can cleanly dispose of the chart by invoking the delete method on the `SciChartSurface`, as described in the [SciChartSurface API documentation](https://www.scichart.com/documentation/js/v5/2d-charts/surface/new-scichart-surface/#scichartsurfacecreate). This methodical approach ensures optimal performance and memory usage, making it a strong foundation for integrating SciChart.js into broader JavaScript applications.",
            },
            react: {
                subtitle:
            ...
            angular: {
                ...
                markdownContent:
                    "## Angular Band Series Chart in Angular\n\n### Overview\nThis Angular example demonstrates how to integrate SciChart.js into a modern Angular application using a standalone component. The chart renders two band series with distinct visual styling and interactive capabilities, allowing for a high-performance band or High-Low Fill representation.\n\n### Technical Implementation\nThe chart is asynchronously initialized within an Angular standalone component that leverages the [ScichartAngularComponent](https://www.npmjs.com/package/scichart-angular) to simplify integration. The example uses the `SciChartSurface` to create numeric X and Y axes, and plots two band series using the [FastBandRenderableSeries](https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/fast-band-renderable-series/) class combined with an `XyyDataSeries` for supplying X, Y, and Y1 values. Each series is enhanced with a [SweepAnimation](https://www.scichart.com/documentation/js/v5/2d-charts/animations-api/series-startup-animations/) for smooth rendering and customized further using gradient fills provided by [GradientParams](https://www.scichart.com/documentation/js/v5/typedoc/classes/gradientparams.html).\n\n### Features and Capabilities\nThe implementation highlights real-time update capabilities and advanced styling customizations. One series uses semi-transparent fills with distinct stroke colors, while the other applies a linear gradient fill effect to both the upper and lower components of the band. Interactive modifiers such as the [ZoomExtentsModifier](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/zooming-and-panning/zoom-pan-modifier/), `ZoomPanModifier`, and `MouseWheelZoomModifier` are incorporated to provide intuitive zooming and panning, ensuring a responsive and engaging user experience.\n\n### Integration and Best Practices\nBy utilizing a standalone Angular component, this example adheres to modern Angular component design practices, simplifying the integration process. Developers can refer to the [Angular standalone components guide](https://angular.io/guide/standalone-components) for further information on structuring such components within an Angular module. The asynchronous initialization pattern used here not only improves performance by offloading heavy tasks but also enhances the overall responsiveness of the application. For more insights into setting up a SciChart.js app with Angular, the [SciChart JS Getting Started guide](https://www.scichart.com/getting-started/scichart-javascript/) provides additional best practices.\n\nThis example serves as a strong foundation for integrating high-performance SciChart.js charts within Angular applications, combining advanced visualizations with efficient WebGL rendering to build sophisticated data visualizations.",
            },
        },
        documentationLinks: [
            {
                href: "https://www.scichart.com/documentation/js/v5/2d-charts/chart-types/fast-band-renderable-series/",
                title: "The specific page for the JavaScript Band Series documentation will help you to get started",
