import type { GridStack, GridStackOptions, GridStackWidget } from "../gridStack/gridstack";
...
export const GridStackContext = createContext<{
  ...
  saveOptionsToFile: () => void; // Saves the current layout, not the initial options
  loadOptionsFromFile: () => void; // Loads a layout from a JSON file
  removeAll: () => void;
