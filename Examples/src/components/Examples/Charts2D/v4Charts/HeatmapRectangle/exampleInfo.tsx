    ...
    //// This metadata is computer generated - do not edit!
    {
        ...
        frameworks: {
            javascript: {
                ...
                markdownContent:
                    "## Heatmap Rectangle Chart - JavaScript\n\n### Overview\nThis example demonstrates how to create a **heatmap-style visualization** using rectangular series in SciChart.js. It replicates heatmap functionality by leveraging the [FastRectangleRenderableSeries](https://www.scichart.com/documentation/js/v4/typedoc/classes/fastrectanglerenderableseries.html) with a custom palette provider for color mapping.\n\n### Technical Implementation\nThe implementation uses an [XyzDataSeries](https://www.scichart.com/documentation/js/v4/typedoc/classes/xyzdataseries.html) to store 3D data points, where Z-values represent heatmap intensity. A custom `HeatmapPaletteProvider` extends [DefaultPaletteProvider](https://www.scichart.com/documentation/js/v4/typedoc/classes/defaultpaletteprovider.html) to map values to colors using either gradient or discrete steps. The chart is initialized asynchronously with hidden axes, focusing purely on the heatmap visualization.\n\n### Features and Capabilities\nThe example showcases dynamic data generation with `generateExampleData()`, supporting real-time updates. Interactive features include [ZoomPanModifier](https://www.scichart.com/documentation/js/v4/typedoc/classes/zoompanmodifier.html), [ZoomExtentsModifier](https://www.scichart.com/documentation/js/v4/typedoc/classes/zoomextentsmodifier.html), and a custom tooltip via [CursorModifier](https://www.scichart.com/documentation/js/v4/typedoc/classes/cursormodifier.html). The color mapping system supports both gradient and discrete modes through the `noGradientColor` and `getGradientColor` functions.\n\n### Integration and Best Practices\nThe implementation follows JavaScript best practices with async initialization and proper resource cleanup. The heatmap data is efficiently transformed and flattened for optimal performance with large datasets. Developers can adjust the `heatmapWidth` and `heatmapHeight` parameters to control resolution.",
            },
            react: {
                subtitle:
                ...
                metaDescription: null,
                markdownContent:
                    "## Heatmap Rectangle Chart - React\n\n### Overview\nThis React example creates a **heatmap visualization** using SciChart's [FastRectangleRenderableSeries](https://www.scichart.com/documentation/js/v4/typedoc/classes/fastrectanglerenderableseries.html) within a React component. The implementation leverages the `<SciChartReact/>` wrapper for seamless integration.\n\n### Technical Implementation\nThe chart is initialized via the `initChart` prop, which points to the `drawExample` function. This function creates a [SciChartSurface](https://www.scichart.com/documentation/js/v4/typedoc/classes/scichartsurface.html) with hidden axes and configures the rectangular series with column modes set to `EColumnMode.Start` and `EColumnYMode.TopHeight`. The custom `HeatmapPaletteProvider` handles color mapping, supporting both gradient and discrete modes.\n\n### Features and Capabilities\nThe component demonstrates React-friendly heatmap rendering with interactive features including zooming, panning, and value inspection via tooltips. The `setChart` function allows dynamic toggling between gradient and discrete color modes. The implementation uses React's component lifecycle for efficient resource management.\n\n### Integration and Best Practices\nThe example follows React best practices by encapsulating chart logic in a separate function and using the [SciChartReact](https://www.scichart.com/documentation/js/v4/typedoc/globals.html) component for integration. The className prop applies responsive styling through CSS modules. Developers can extend this pattern for state-controlled heatmap updates.",
            },
            angular: {
                subtitle:
