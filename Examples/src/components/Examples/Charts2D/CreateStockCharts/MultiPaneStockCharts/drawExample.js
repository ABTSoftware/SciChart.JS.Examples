...
export const getChartsInitializationAPI = () => {
    ...
    const opacity = "AA";
    // CHART 1
    const drawPriceChart = async (rootElement) => {
    ...
    // CHART 2 - MACD
    const drawMacdChart = async (rootElement) => {
        const { wasmContext, sciChartSurface } = await SciChartSurface.create(rootElement, {
            // prevent default size settings
            disableAspect: true,
            theme: appTheme.SciChartJsTheme,
    ...
    // CHART 3 - RSI
    const drawRsiChart = async (rootElement) => {
        const { wasmContext, sciChartSurface } = await SciChartSurface.create(rootElement, {
            // prevent default size settings
            disableAspect: true,
            theme: appTheme.SciChartJsTheme,
    ...
    const drawOverview = (mainSurface) => async (rootElement) => {
...
class VolumePaletteProvider {
    ...
    volumeUpArgb;
    volumnDownArgb;
    constructor(priceData, volumeUpColor, volumeDownColor) {
