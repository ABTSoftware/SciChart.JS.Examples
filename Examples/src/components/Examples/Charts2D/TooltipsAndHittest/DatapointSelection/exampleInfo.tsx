    ...
    //// This metadata is computer generated - do not edit!
    {
        ...
        frameworks: {
            javascript: {
                ...
                markdownContent:
                    "## Datapoint Selection – JavaScript\n\n### Overview\nThis example demonstrates how to implement data point selection in SciChart.js Charts using JavaScript. The example creates several spline line series with simulated data and enables interactive selection of data points. It leverages the SciChart.js core components such as axes, renderable series, annotations, and modifiers to provide a rich, interactive charting experience.\n\n### Technical Implementation\nThe chart is set up by creating a `SciChartSurface` with a WebAssembly context (see [Creating a new SciChartSurface and loading Wasm](https://www.scichart.com/documentation/js/v5/2d-charts/surface/new-scichart-surface/) for more details). Two numeric axes are added to the surface and a number of spline line renderable series are created using an `XyDataSeries`. Each series uses custom point markers styled according to an automatic color scheme. In addition, a custom data label provider built with the `LineSeriesDataLabelProvider` is implemented to conditionally display labels only for selected points based on `metadata`. For handling data point selection, the [DataPointSelectionModifier](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/selection/data-point-selection/) is added to the chart. Its event subscription mechanism is used to update a table of selected points via a callback when selection changes occur. The example also makes use of the `DataPointSelectionPaletteProvider` to customize the stroke and fill of selected data points denoted by `metadata.isSelected`.\n\n### Features and Capabilities\nThis chart supports interactive selection via click, CTRL+CLICK, and click & drag actions. Selected data points display custom data labels that include the x and y values, and are visually distinguished by a custom palette. The example also includes text annotations (configured with `TextAnnotation`) to guide the user, and a legend provided by the [LegendModifier](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/miscellaneous-modifiers/legend-modifier/) to give context to the rendered series. Overall, the example illustrates how multiple renderable series can be enhanced with conditional data labels, palette customization, and interactive selection behavior.\n\n### Integration and Best Practices\nEven though this example is implemented in JavaScript, it demonstrates several best practices applicable across different frameworks. It uses the native event subscription model provided by SciChart.js modifiers (see [Common ChartModifiers Features](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/common-features/)) and leverages the high-performance WebAssembly context for rendering. Developers can use these techniques to efficiently manage real-time interactions and achieve optimal performance when handling multiple series and complex user interactions. Overall, the implementation provides a solid foundation that can be extended or integrated into larger applications.",
            },
            react: {
                subtitle:
                    "Demonstrates how to **Select Data Points** on a chart using SciChart.js, High Performance JavaScript Charts",
            ...
            angular: {
                ...
                metaDescription:
                    "Demonstrates the DatapointSelectionModifier, which provides a UI to select one or many data points, and works with DataPointSelectionPaletteProvider to change the appearance of selected points",
                markdownContent:
                    "## Angular Chart Data Point Selection\n\n### Overview\nThis example demonstrates how to interactively select data points on a SciChart.js chart within an Angular application. It utilizes the `DataPointSelectionModifier` to allow users to click, CTRL+click, or drag-select points, providing immediate visual feedback on selected data. For further details on data point selection, refer to the [DataPoint Selection documentation](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/selection/data-point-selection/).\n\n### Technical Implementation\nThe chart is initialized asynchronously using an async function that sets up numeric axes and multiple `SplineLineRenderableSeries`. Each series is enhanced with conditional styling via the `DataPointSelectionPaletteProvider` and a custom data label provider that only displays labels for selected points, denoted by `metadata.isSelected`. The implementation subscribes to the `selectionChanged` event, ensuring that the Angular component state is updated in real time.\n\n### Features and Capabilities\nThis example offers real-time updating of selected data points, which are displayed in an adjacent data table. It includes advanced features such as customizable point markers, dynamic annotations, and an integrated legend, all designed to enhance user interaction. The conditional styling provided by the `DataPointSelectionPaletteProvider` which is key to differentiating selected points, as detailed in the [DataPoint Selection documentation](https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/selection/data-point-selection/).\n\n### Integration and Best Practices\nIntegrating SciChart.js within an Angular environment is streamlined by embedding the chart initialization into an Angular component. The example demonstrates efficient event handling and state synchronization using Angular's data binding techniques. Developers can benefit from the [scichart-angular package](https://www.npmjs.com/package/scichart-angular) for additional guidance on best practices. Moreover, exploring [Performance Tips & Tricks](https://www.scichart.com/documentation/js/v5/2d-charts/performance-tips/performance-tips-and-tricks/) can help optimize the performance of applications handling multiple renderable series.\n",
            },
        },
        documentationLinks: [
            {
                href: "https://www.scichart.com/documentation/js/v5/2d-charts/chart-modifier-api/selection/data-point-selection/",
