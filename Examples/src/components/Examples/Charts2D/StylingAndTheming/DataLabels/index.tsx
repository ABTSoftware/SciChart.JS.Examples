...
const drawExample = async () => {
    ...
    sciChartSurface.renderableSeries.add(
        new SplineLineRenderableSeries(wasmContext, {
            dataSeries: new XyDataSeries(wasmContext, {
                ...
                metadata: data1.xValues.map(
                    (x, i) => ({ isSelected: false, text: labels[(i - 1) / 2] } as IPointMetadata)
                )
            }),
    ...
    sciChartSurface.annotations.add(
        new NativeTextAnnotation({
            ...
            y1: 14,
            text:
                "Series with 200 points. Using pointGapThreshold = 1 to show labels when zoomed in enough for there to be space for them",
