...
export const drawExample = async (rootElement: string | HTMLDivElement) => {
  ...
  const addSeries = (stroke: string, opacity: number) => {
    ...
    const { xValues, yValues } = ExampleDataProvider.getNoisySinewave(
      500,
      900,
      7,
      amplitude,
  ...
  const reassignRenderableSeries = () => {
    ...
    ExampleDataProvider.fillNoisySinewave(
      500,
      900,
      7,
      amplitude,
    ...
    timerId = setTimeout(reassignRenderableSeries, 0);
  };
