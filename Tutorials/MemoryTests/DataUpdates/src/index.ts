...
let dataSeriesCapacity = 2000000;
let dataChunkSize = 1000000;
...
const init2dChartWithSharedContext = async (rootElement: HTMLDivElement) => {
  ...
  const xyDataSeries = new XyDataSeries(wasmContext, {
    capacity: useCapacity ? dataSeriesCapacity : undefined,
    containsNaN: false,
    isSorted: true,
  });
  ...
  const appendData = () => {
    for (let i = 0; i < dataChunkSize; i++, dataPointCount++) {
  ...
  const removeData = () => {
    // Remove some points from the data series
    xyDataSeries.removeRange(0, dataChunkSize);
  };
...
(document.querySelector("#useCapacity") as HTMLInputElement).onclick =
  function (ev: MouseEvent) {
    useCapacity = (ev.target as HTMLInputElement).checked;
