...
Demonstrates SciChart's **Simple Server Validation v2** flow using ASP.NET Core — server-side license validation using only `System.Security.Cryptography.HMACSHA256`, with no NuGet package and no native DLL.
...
2. **Set the Server Secret**
   Edit `Program.cs` and replace `SCICHART_SERVER_SECRET` with your Server Secret.
