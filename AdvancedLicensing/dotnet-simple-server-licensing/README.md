# Simple Server Licensing Example (.NET)

Demonstrates SciChart's **Simple Server Validation** flow using ASP.NET Core — server-side license validation using only `System.Security.Cryptography.HMACSHA256`, with no NuGet package and no native DLL.
...
2. **Set the Server Secret**
   Edit `Program.cs` and replace the `SCICHART_SERVER_SECRET` value with your Server Secret.
