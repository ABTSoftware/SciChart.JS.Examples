async function customLegend(divElementId) {
  ...
  for (let i = 0; i < 4; i++) {
    sciChartSurface.renderableSeries.add(new FastLineRenderableSeries(wasmContext, {
      stroke: AUTO_COLOR,
      dataSeries: new XyDataSeries(wasmContext, { dataSeriesName: `Series ${i + 1}`, xValues, yValues: xValues.map(x => x + i) })
    }))
  }
  ...
  legendModifier.sciChartLegend.getLegendItemHTML = (orientation, showCheckboxes, showSeriesMarkers, item) => {
    ...
    // Add another label
    str += `<label style="margin-left: 8px;">Last value: ${series.dataSeries.getNativeYValues().get(series.dataSeries.count()-1)}</label>`;
    str += `</span>`;
    return str;
  };
  ...
  legendModifier.sciChartLegend.getLegendHTML = (placement, textColor, backgroundColor, margin, orientation, showCheckboxes, showSeriesMarkers, items) => {
      let html = getHtml.call(legendModifier.sciChartLegend, placement, textColor, backgroundColor, margin, orientation, showCheckboxes, showSeriesMarkers, items);
      // Center the legend
