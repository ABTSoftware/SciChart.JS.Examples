async function basicSeriesSelection(divElementId) {
  ...
  sciChartSurface.renderableSeries.add(
    new FastLineRenderableSeries(wasmContext, {
      ...
      onSelectedChanged: (sourceSeries) => {
        ...
        sourceSeries.pointMarker.fill = sourceSeries.isSelected
          ? "white"
          : "SteelBlue";
        sourceSeries.pointMarker.stroke = sourceSeries.isSelected
          ? "white"
          : "LightSteelBlue";
      },
    })
  ...
  sciChartSurface.renderableSeries.get(0).selected.subscribe((args) => {
    console.log(
      `Series ${args.sourceSeries.dataSeries.dataSeriesName} was ${
        args.isSelected ? "" : "de"
      }selected`
    );
  });
      ...
      series: [
        {
          ...
          options: {
            ...
            // using onSelectedChanged callback, change the style of the series on selection
            onSelectedChanged: (sourceSeries) => {
              sourceSeries.stroke = sourceSeries.isSelected
                ? "white"
                : "SteelBlue";
              sourceSeries.pointMarker.fill = sourceSeries.isSelected
                ? "white"
