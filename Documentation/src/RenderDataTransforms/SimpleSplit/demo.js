...
async function simpleSplit(divElementId) {
    const { sciChartSurface, wasmContext } = await scichart_1.SciChartSurface.create(divElementId, { theme: new scichart_1.SciChartJsNavyTheme() });
    sciChartSurface.xAxes.add(new scichart_1.NumericAxis(wasmContext));
    sciChartSurface.yAxes.add(new scichart_1.NumericAxis(wasmContext, { growBy: new scichart_1.NumberRange(0.1, 0.1) }));
    // #region ExampleB
    ...
    const selectedPointDrawingProvider = new scichart_1.PointMarkerDrawingProvider(wasmContext, renderableSeries, (ps) => ps.y1Values);
    ...
    sciChartSurface.chartModifiers.add(new scichart_1.DataPointSelectionModifier({
