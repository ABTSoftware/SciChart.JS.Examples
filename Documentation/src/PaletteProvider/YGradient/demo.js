...
async function lineChartWithyGradient(divElementId) {
  ...
  const yValues = (0, scichart_1.makeIncArray)(
    250,
    1,
    (y) => Math.sin(y * 0.05) + Math.sin(y * 0.01)
  );
  // #region ExampleA
  const yGradientPalette = scichart_1.PaletteFactory.createYGradient(
    wasmContext,
    new scichart_1.GradientParams(
      new scichart_1.Point(0, 0),
      new scichart_1.Point(0, 1),
      [
        { offset: 0, color: "blue" },
