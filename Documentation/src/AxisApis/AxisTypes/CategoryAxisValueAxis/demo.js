async function categoryAxisVsValueAxis() {
  ...
  const addLineSeries = (sciChartSurface, xValues, yValues, stroke, dataSeriesName) => {
    sciChartSurface.renderableSeries.add(new FastLineRenderableSeries(sciChartSurface.webAssemblyContext2D, {
      dataSeries: new XyDataSeries(sciChartSurface.webAssemblyContext2D, { xValues, yValues, dataSeriesName}),
      stroke,
      strokeThickness: 2,
      pointMarker: new EllipsePointMarker(sciChartSurface.webAssemblyContext2D, { width: 7, height: 7, fill: stroke, stroke}),
    }));
  };
