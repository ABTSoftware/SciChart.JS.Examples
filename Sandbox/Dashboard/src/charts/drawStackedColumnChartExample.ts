...
export const drawStackedColumnChartExample = async () => {
  ...
  const icons = await createImagesArrayAsync(
    ICONS.map((iconObj: IIcon) => iconObj.image)
  );
  ...
  xAxis.labelProvider.getLabelTexture = (
    ...
    if (!isNaN(index)) {
      ...
      if (icon) {
        return textureManager.createTextureFromImage(
          icon,
          ICONS[index].width,
          ICONS[index].height
        );
  ...
  const incomeDataSeries = new XyDataSeries(wasmContext, {
    xValues,
    yValues: income,
    seriesName: "Income",
  });
  const taxesDataSeries = new XyDataSeries(wasmContext, {
    xValues,
    yValues: taxes,
    seriesName: "Taxes",
  });
  ...
  sciChartSurface.chartModifiers.add(
    new RolloverModifier({ rolloverLineStroke: colors.blueSchema[300] })
  );
  sciChartSurface.domCanvas2D.addEventListener(
    ...
    (mouseEvent: MouseEvent) => {
      ...
      let hitTestResult: HitTestInfo = rendSeries1.hitTestProvider.hitTest(
        premultipliedX,
        premultipliedY
      );
      if (!hitTestResult.isHit) {
        hitTestResult = rendSeries2.hitTestProvider.hitTest(
          premultipliedX,
          premultipliedY
        );
      }
