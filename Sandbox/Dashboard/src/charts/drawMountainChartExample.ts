...
export const drawMountainChartExample = async () => {
  ...
  const sinSeries: FastMountainRenderableSeries =
  ...
  const cosSeries: FastMountainRenderableSeries =
    new FastMountainRenderableSeries(wasmContext, {
      stroke: colors.transparent,
      strokeThickness: 0,
      zeroLineY: 0.0,
  ...
  const sinData = new XyDataSeries(wasmContext, { seriesName: "Cos(x)" });
  const cosData = new XyDataSeries(wasmContext, { seriesName: "Sin(x)" });
