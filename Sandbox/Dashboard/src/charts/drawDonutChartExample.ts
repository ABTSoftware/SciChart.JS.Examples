...
export const drawDonutChartExample = async () => {
  const sciChartPieSurface = await SciChartPieSurface.create(donutChart, {
    theme,
  });
  ...
  getDonutChartData().forEach((data) =>
    sciChartPieSurface.pieSegments.add(new PieSegment(data))
  );
