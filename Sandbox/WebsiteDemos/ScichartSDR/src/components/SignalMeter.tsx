...
import { buildSignalMeterGradientStops, toCssLinearGradient } from "./signalPalette";
...
export function SignalMeter({ signalPeakDb, minDb, maxDb }: SignalMeterProps) {
  ...
  const gradient = toCssLinearGradient(buildSignalMeterGradientStops(minDb, maxDb));
  ...
  return (
    <div className="smeter-wrap">
      ...
      <Typography className="smeter-value">{signalPeakDb.toFixed(0)} dB</Typography>
    </div>
  );
}
