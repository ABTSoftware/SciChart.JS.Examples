...
export function useReceiverSettings() {
  ...
  const [upconverterFrequencyHz, setUpconverterFrequencyHz] = useState(
    savedSettings.upconverterFrequencyHz ?? 100_000_000
  );
  ...
  const [manualGain, setManualGain] = useState(
    savedSettings.manualGain ?? false
  );
  ...
  const [fftSize, setFftSize] = useState(savedSettings.fftSize ?? FFT_SIZE);
