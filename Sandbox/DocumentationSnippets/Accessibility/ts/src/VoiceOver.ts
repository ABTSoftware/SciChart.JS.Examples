...
const isPointWithinAxis = (point: Point, axis: AxisBase2D) => {
  return (
    point.x * DpiHelper.PIXEL_RATIO > axis.viewRect.left &&
...
export async function voiceOverDataSeries(divElementId: string) {
  ...
  const dataSeries = new XyDataSeries(wasmContext, {
    seriesName: "Primary Data Series",
    xValues: [1, 2, 3, 4, 5],
    yValues: [8, 6, 7, 2, 16],
  });
  ...
  const voiceOverDataPoint = (
    series: IRenderableSeries,
    hitTestInfo: HitTestInfo
  ) => {
    const { hitTestPointValues } = hitTestInfo;
    const xCoordValue = series.xAxis.labelProvider.formatLabel(
      hitTestPointValues.x
    );
    const yCoordValue = series.yAxis.labelProvider.formatLabel(
...
export async function voiceOverVisibleRange(divElementId: string) {
  const { wasmContext, sciChartSurface } = await SciChartSurface.create(
    divElementId
  );
