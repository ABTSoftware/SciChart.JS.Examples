...
import { TSharedDataDefinition } from "scichart/Builder/types/DataSeriesDefinitions";
import { ESeriesType } from "scichart/types/SeriesType";
import { XyDataSeries } from "scichart/Charting/Model/XyDataSeries";

export async function drawChartWithSharedDataSeries(divElementId: string) {
  const chartDefinition: ISciChart2DDefinition = {
    series: [
      {
  ...
  const sharedData: TSharedDataDefinition = {
    x: [1, 2, 3, 4, 5],
    col: [8, 2, 3, 7, 10],
    line: [10, 6, 7, 2, 16],
  };
  const { sciChartSurface, wasmContext } = await chartBuilder.build2DChart(
    divElementId,
    { ...chartDefinition, sharedData }
  );
}

export async function drawChartWithManuallyCreatedDataSeries(
  divElementId: string
) {
  const chartDefinition: ISciChart2DDefinition = {
    series: [
...
export async function drawChartWithBuiltRenderableSeries(divElementId: string) {
