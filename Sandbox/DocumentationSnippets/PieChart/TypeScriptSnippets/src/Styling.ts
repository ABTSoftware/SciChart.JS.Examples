...
export async function pieChartLabelFormat(divElementId: string) {
  ...
  sciChartPieSurface.labelProvider.getSegmentText = (
    segment: IPieSegment,
    total
  ) =>
    `<span>${segment.text}<span><br/><span>${segment.value.toFixed(0)} (${(
      (100 * segment.value) /
      total
    ).toFixed(1)}%)</span>`;
  ...
  pieSegment2.labelProvider.getSegmentText = (segment: IPieSegment, total) =>
    "Some Apples";
  ...
  pieSegment4.labelProvider.formatLabel = (dataValue: number) =>
    dataValue.toFixed(2) + "%";
