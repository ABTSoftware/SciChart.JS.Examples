...
export default async function init2dFifthChart(id: string) {
    ...
    const dataG = getDataDiagonal(xValues, yValues.map(el => el < 3 ? el : el + 3), 100);
    const dataB = getDataDiagonal(xValues, yValues.map(el => el < 3 ? el : el + 6), 100);
