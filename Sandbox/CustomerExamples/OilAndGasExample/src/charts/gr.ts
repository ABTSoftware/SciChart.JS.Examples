...
export default async function initGrChart(
  ...
  const lineDataSeries = getRandomSinewave(
    wasmContext,
    0,
    Math.random() * 3,
    Math.random() * 50,
  ...
  lineDataSeries.dataSeries.seriesName = "GR";
  ...
  for (let i = 0; i < pointsCount; i += pointsCount / 10) {
    dashedDataSeries.append(
      i,
      Math.random() * (lineDataSeries.min + 1 - lineDataSeries.min) +
        lineDataSeries.min
    );
  }
  ...
  return (
    ...
    if (name === "GR") {
      ...
      const secondaryColor =
        sciChartSurface.renderableSeries.asArray()[1].stroke;
      str += `
