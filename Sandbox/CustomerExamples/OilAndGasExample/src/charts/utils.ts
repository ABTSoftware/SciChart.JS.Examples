import { MouseWheelZoomModifier, SciChartSurface } from "scichart";
...
export function generateModifiers(
  ...
  sciChartSurface.chartModifiers.add(
    new RolloverModifier({ modifierGroup: "first" })
  );
  sciChartSurface.chartModifiers.add(
    new ZoomPanModifier({ modifierGroup: "first", excludedYAxisIds: [yAxisId] })
  );
  sciChartSurface.chartModifiers.add(new MouseWheelZoomModifier());
  const legendModifier = new LegendModifier({
    showCheckboxes: true,
    placementDivId: `${id}-legend`,
  });
