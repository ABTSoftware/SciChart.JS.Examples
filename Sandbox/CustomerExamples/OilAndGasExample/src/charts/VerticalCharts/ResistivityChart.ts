...
export const drawResistivityChart = async () => {
  ...
  const movingAverage20DataSeries = new XyMovingAverageFilter(dataSeries, {
    length: 20,
    dataIsSortedInX: true,
  });
  ...
  const legendModifier = new LegendModifier({
    placementDivId: `resistivity-legend`,
  });
...
const generateResistivityLegend = (
  ...
  return `
    ...
    <span class="scichart__legend-line" style="border-top: 2px dashed ${
      appTheme.ResistivityLineStroke2
    }"></span>
        ...
        <span class="scichart__legend-line" style="border-top: 2px solid ${
          appTheme.ResistivityLineStroke
        }"></span>
        <div class="legend-text-item">
            <span>${0}</span>
