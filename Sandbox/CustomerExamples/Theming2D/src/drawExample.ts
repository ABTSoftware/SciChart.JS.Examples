...
const drawPieChart = async (rootElement: string | HTMLDivElement) => {
    ...
    const pieColors = [colorPalette.color1, colorPalette.color2, colorPalette.color3, colorPalette.color4];
...
const drawPolarChart = async (rootElement: string | HTMLDivElement) => {
    ...
    sciChartSurface.yAxes.add(new PolarNumericAxis(wasmContext, {
        ...
        visibleRange: new NumberRange(Math.min(...DATA_UK.data), Math.max(...DATA_UK.data) + 4),
    ...
    function generatePolarTraces(numPoints = 300) {
        ...
        return {
            ...
            data: [
                ...
                {
                    name: "Butterfly",
                    yValues: normalize(theta.map((t) => Math.exp(Math.cos(t)) - 2 * Math.cos(4 * t) + Math.pow(Math.sin(t / 12), 5))),
